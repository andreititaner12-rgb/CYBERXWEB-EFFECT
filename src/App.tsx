...
// Тяжёлые секции (three.js ~600KB и Leaflet ~150KB) грузим лениво:
// их код и GPU-контексты не нужны, пока пользователь не доскроллил
...
export function App() {
  ...
  // Idle-предзагрузка тяжёлых ленивых чанков (three.js, Leaflet):
  // грузим код в фоне, когда браузер свободен — к моменту скролла пользователя
  // до секций «Железо» и «Карта» модули уже в кэше (мгновенное монтирование).
  useEffect(() => {
    const idle = (window as unknown as { requestIdleCallback?: (cb: () => void, opts?: { timeout: number }) => number }).requestIdleCallback;
    const preload = () => {
      import('./components/HardwareVisualizer').catch(() => {});
    ...
    >
      ...
      >
        ...
        <main className="relative z-10 pt-4 pb-12">
          ...
          <LazyMount id="location" estimateHeight={700} className="scroll-mt-24" rootMargin="1100px 0px">
            <Suspense fallback={null}>
              <LocationMapSection
                onOpenBooking={(arenaId) => handleOpenBooking(arenaId)}
                arenasList={liveArenas}
