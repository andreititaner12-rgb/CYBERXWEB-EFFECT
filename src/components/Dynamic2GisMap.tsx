...
export const Dynamic2GisMap: React.FC<Dynamic2GisMapProps> = ({
  ...
  const isFirstRunRef = useRef<boolean>(false);
  ...
  useEffect(() => {
    ...
    L.tileLayer(
      `https://{s}.basemaps.cartocdn.com/rastertiles/dark_all/{z}/{x}/{y}.png?key=${CARTO_API_KEY}`,
      {
        ...
        maxZoom: 20,
        minZoom: 9,
        // Держим минимум тайлов вне вьюпорта: было 8 рядов — огромный запас
        // текстур для композитора (разрывы на слабых GPU). 1 ряд незаметен
        // глазу при панораме, но в разы легче.
        keepBuffer: 1,
  ...
  useEffect(() => {
    ...
    // Первый прогон (монтирование карты): камера УЖЕ стоит на нужной точке
    // (задана в L.map(...)) — анимированный flyTo сюда запускал 1.3-секундный
    // полёт прямо в момент, когда ленивая секция появляется на экране:
    // тайлы грузятся + трансформ панелей + маркеры = рывки и артефакты на GPU.
    if (!isFirstRunRef.current) {
      isFirstRunRef.current = true;
    } else {
      // Smooth fly to the active club coordinates (только по действию пользователя)
  ...
  const handleFitAll = () => {
    ...
    map.flyToBounds(bounds, { padding: [60, 60], duration: 0.85 });
  };
