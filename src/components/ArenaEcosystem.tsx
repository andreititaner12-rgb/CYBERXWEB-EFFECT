...
export const ArenaEcosystem: React.FC<ArenaEcosystemProps> = ({
  ...
  // Обзор клуба: показанный клуб + фаза затухания (плавная смена без exit-клонов)
  const [displayArenaId, setDisplayArenaId] = useState<string>(selectedArenaId || 'cyberx-arena');
  const [isDeepDiveFading, setIsDeepDiveFading] = useState(false);
  const fadeTimerRef = useRef<number | null>(null);

  // Gallery slider state for deep-dive section
  const [galleryIndex, setGalleryIndex] = useState<number>(0);
  // Предыдущее фото держим смонтированным, пока оно затухает (кроссфейд).
  // Рендерим ТОЛЬКО текущее + предыдущее — вместо всех 8-10 фото сразу
  // (моментальная монтировка целого стека фото при смене клуба перегружала
  // растеризацию и роняла вкладку на слабых GPU).
  const [prevGalleryIndex, setPrevGalleryIndex] = useState<number>(0);
  const [isHoveringGallery, setIsHoveringGallery] = useState<boolean>(false);

  const currentArena = arenasList.find((a) => a.id === displayArenaId) || arenasList[1] || arenasList[0];
  ...
  useEffect(() => {
    setGalleryIndex(0);
    setPrevGalleryIndex(0);
  }, [displayArenaId]);

  // Отпускаем предыдущее фото после завершения кроссфейда (700мс)
  useEffect(() => {
    const t = setTimeout(() => setPrevGalleryIndex(galleryIndex), 750);
  ...
  const handleSelectArena = (arenaId: string) => {
    ...
    // Тот же клуб — просто прокручиваем к обзору
    if (arenaId === displayArenaId) {
      const el = document.getElementById('arena-deep-dive');
      if (el) smoothScrollTo(el);
      return;
    }

    // Другой клуб: блок плавно ЗАТУХАЕТ, затем меняется контент и ПЛАВНО ВЪЕЗЖАЕТ.
    // (Exit-клоны framer-motion на этом большом блоке перегружали растеризацию —
    //  чистый CSS fade даёт ту же плавность без двойных блоков в DOM.)
    if (fadeTimerRef.current) window.clearTimeout(fadeTimerRef.current);
    setIsDeepDiveFading(true);
    fadeTimerRef.current = window.setTimeout(() => {
      setDisplayArenaId(arenaId);
      setIsDeepDiveFading(false);
      const el = document.getElementById('arena-deep-dive');
      if (el) smoothScrollTo(el);
    }, 260);
  };
  ...
  return (
    <section id="arenas" className="relative py-8 sm:py-12 bg-transparent scroll-mt-24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          {arenasList.map((arena) => {
            ...
            return (
              <CardContainer
                key={arena.id}
                containerClassName={`w-full transition-all duration-300 ease-out ${
              ...
              >
                ...
                >
                  ...
                  <CardItem translateZ={35} className="pt-4 border-t border-white/[0.08] flex items-center justify-between gap-3 relative z-10 font-mono mt-auto w-full">
                    <div>
        ...
        {/* Selected Arena Deep Dive: плавное затухание старого и въезд нового обзора */}
        <div id="arena-deep-dive" className="scroll-mt-28">
          <div
            key={displayArenaId}
            className={`glass-card rounded-3xl border border-white/[0.12] overflow-hidden shadow-2xl relative bg-[#07070d]/98 cyberx-deepdive ${isDeepDiveFading ? 'cyberx-deepdive-fading' : ''}`}
          >
              ...
              >
                ...
                <div className="absolute inset-0 overflow-hidden bg-black pointer-events-none">
                  {galleryPhotos.map((photo, idx) => {
                    ...
                    if (!isActive && idx !== prevGalleryIndex) return null;
                    return (
                      <div
                        key={idx}
                        className={`absolute inset-0 transition-opacity duration-700 ease-in-out ${
                          isActive ? 'gallery-photo-in z-10' : 'opacity-0 z-0 pointer-events-none'
                        }`}
                      >
                        <img
                          src={photo}
                          ...
                          className={`w-full h-full object-cover object-center ${
                            isActive ? 'gallery-kenburns' : 'scale-100'
                          }`}
                          loading="lazy"
                          decoding="async"
                        />
