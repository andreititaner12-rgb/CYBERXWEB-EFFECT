...
export const Mouse3DViewer: React.FC = () => {
  ...
  // Ленивая инициализация WebGL: контекст/сцена создаются ТОЛЬКО при первом
  // реальном показе вьюера (вкладка «Мыши» открыта И видна на экране).
  // Раньше WebGL создавался в момент монтирования секции «Железо» (середина
  // блока «Зоны») — на слабых GPU это роняло драйвер и деградировала вся
  // вкладка до перезагрузки. Активация однажды — дальше контекст живёт,
  // повторные показы вкладки мгновенны (без пересоздания).
  const [activated, setActivated] = useState<boolean>(false);

  // Триггер активации: первый показ элемента в вьюпорте
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Кап 1.75 вместо 2: на Retina-экранах заметно дешевле растеризация, визуально неотличимо
    ...
    // 6. Animation Loop — останавливается, когда секция вне вьюпорта
    // (иначе WebGL рендерит 60 FPS бесконечно, нагружая GPU на всей странице)
    ...
    const applySize = () => {
      ...
      // Скрытый (display:none) контейнер даёт 0x0 — пропускаем,
      // иначе renderer.setSize(0,0) «схлопнет» канвас.
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            // Вкладка показана (в т.ч. первый показ после CSS-hidden):
            // пересчитываем размер — при монтировании в скрытом состоянии
            // использовался фолбэк 360x280, а окно могло поменяться.
            applySize();
            startLoop();
          } else {
            stopLoop();
    ...
    return () => {
      ...
      // Полное освобождение GPU-ресурсов сцены
      scene.traverse((obj) => {
