...
export const CustomCrosshairCursor: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // GPU lerp loop с автоматической остановкой в покое:
    // пока курсор не двигается и позиция сошлась — rAF НЕ крутится (0% CPU в idle)
    const render = () => {
      ...
      const settled =
        Math.abs(targetRef.current.x - currentPos.current.x) < 0.1 &&
        Math.abs(targetRef.current.y - currentPos.current.y) < 0.1;

      if (settled) {
        requestRef.current = null; // стоп: работа возобновится на следующем mousemove/mousedown
      } else {
